...
public class LDAP {
	...
	private void rellenarDatosUsuario(Persona persona, Attributes attribs) {
		...
		if (persona instanceof Profesor)
			...
			rellenarDatosAlumno(persona, attribs);
	...
	// Metodo auxiliar de rellenarDatosUsuario
	private void rellenarDatosAlumno(Persona persona, Attributes attribs) {

		String value = null;
		Alumno p = (Alumno) persona;

		try {
			value = attribs.get("NIA").toString();
			p.setNIA(value.substring(value.indexOf(":") + 2));
		} catch (Exception e) {
		}
		try {
			value = attribs.get("sexo").toString();
			p.setSexo(value.substring(value.indexOf(":") + 2));
		} catch (Exception e) {
		}
		try {
			value = attribs.get("fechaMatricula").toString();
			p.setFechaMatricula(value.substring(value.indexOf(":") + 2));
		} catch (Exception e) {
		}
		try {
			value = attribs.get("ensenyanza").toString();
			p.setEnsenyanza(value.substring(value.indexOf(":") + 2));
		} catch (Exception e) {
		}
		try {
			value = attribs.get("cicloFP").toString();
			p.setCicloFP(value.substring(value.indexOf(":") + 2));
		} catch (Exception e) {
		}
		try {
			value = attribs.get("curso").toString();
			p.setCurso(value.substring(value.indexOf(":") + 2));
		} catch (Exception e) {
		}
		try {
			value = attribs.get("grupo").toString();
			p.setGrupo(value.substring(value.indexOf(":") + 2));
		} catch (Exception e) {
		}
		try {
			value = attribs.get("turnoFP").toString();
			p.setTurnoFP(value.substring(value.indexOf(":") + 2));
		} catch (Exception e) {
		}
		try {
			value = attribs.get("expGC").toString();
			p.setExpGC(value.substring(value.indexOf(":") + 2));
		} catch (Exception e) {
		}

		p.setPassword(p.getNIA());
	}
